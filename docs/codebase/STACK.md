...
- Group: `sk.innovis`, Artifact: `eranet-server-sepstest`, Version: `1.0-SNAPSHOT`
- Packaging: **WAR**
...
| `maven-war-plugin` | 2.3 | Package WAR, bundle SQL into `WEB-INF/sql/`, set logging profile `seps_profile` |
...
**Target Runtime:** WildFly 26.1.3 (JNDI name pattern `PublicSepsDS` in `persistence.xml`; WildFly path referenced in `configS.bat`)
...
- JTA data source JNDI: `PublicSepsDS`
...
**Logging:**
- Server: WildFly logging profile `seps_profile` set via `MANIFEST.MF` (`maven-war-plugin` configuration)
