...
- Group: `sk.innovis`, Artifact: `eranet-server-vsetest`, Version: `1.0-SNAPSHOT`
- Packaging: **WAR**
...
| `maven-war-plugin` | 2.3 | Package WAR, bundle SQL into `WEB-INF/sql/`, set logging profile `vsds_profile` |
...
**Target Runtime:** WildFly 26.1.3 (JNDI name pattern `EranetVseTestDS` in `persistence.xml`; WildFly path referenced in `configS.bat`)
...
- JTA data source JNDI: `EranetVseTestDS`
...
**Logging:**
- Server: WildFly logging profile `vsds_profile` set via `MANIFEST.MF` (`maven-war-plugin` configuration)
