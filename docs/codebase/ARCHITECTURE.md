...
│  `public-eranet/.../server/service/*Service.java`                    │
│           @Path("webresources")  @ApplicationPath("webresources")    │
│                                                                      │
│  EJB Business/Service layer                                          │
│  `public-eranet/.../server/service/BaseService`                      │
│                                                                      │
│  JPA (EclipseLink) DAO layer                                         │
│  `eranet-dao/.../server/dao/*Dao.java`                               │
│                                                                      │
│  JPA Entities / DTOs                                                 │
│  `eranet-domain/.../server/dto/*.java` (218 cls)                     │
└──────────────────────────────────────────────────────────────────────┘
                              │  JTA / JDBC
                              ▼
┌──────────────────────────────────────────────────────────────────────┐
│   MySQL database  (JNDI: PublicSepsDS / PublicDS)                    │
│   Persistence unit: PublicTestPU                                     │
│   `eranet-domain/src/main/resources/META-INF/persistence.xml`        │
└──────────────────────────────────────────────────────────────────────┘
```
...
| Angular controllers | Screen-level logic, UI state, calling services | `publicERANET-client/app/scripts/controllers/**/*.js` (15 top-level files + subdirs) |
| Angular resource services | `$resource` wrappers mapping to server REST URLs | `publicERANET-client/app/scripts/service/resources/*.js` (171 files) |
...
| `ApplicationConfig.java` | JAX-RS `Application` subclass — registers all REST resource classes | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/ApplicationConfig.java` |
| `BaseService<T>` | Abstract ancestor for all REST services: date formats, EJB injection, find/copy helpers | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/BaseService.java` |
| Concrete `*Service.java` | `@Stateless` EJBs annotated with `@Path`: expose CRUD + query endpoints for each domain object | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/*.java` (188 files) |
| Service API interfaces | Shared service contracts consumed across modules | `publicERANET-server/public-eranet-api/src/main/java/sk/innovis/eranetpublic/server/service/*.java` |
| `BaseDao<T>` | Generic JPA CRUD, criteria-builder queries, pagination | `publicERANET-server/eranet-dao/src/main/java/sk/innovis/eranetpublic/server/dao/BaseDao.java` |
| Concrete `*Dao.java` | Entity-specific queries extending BaseDao | `publicERANET-server/eranet-dao/src/main/java/sk/innovis/eranetpublic/server/dao/*.java` (139 files) |
| JPA entities / DTOs | `@Entity` classes mapped to MySQL tables; also serialized directly as JSON responses | `publicERANET-server/eranet-domain/src/main/java/sk/innovis/eranetpublic/server/dto/*.java` (218 classes) |
| `AuthenticationService` | Form-login, SAML SSO, password reset, registration | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/AuthenticationService.java` |
| SAML/SSO | ÚPVS government SSO + AD/eID (Azure AD SAML) SSO | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/sso/` (`UpvsSsoService.java`/`UpvsSsoApplication.java`, `SsoService.java`/`SsoApplication.java`) |
| WebSocket endpoint | Real-time push notifications over WebSocket at `/websockets/realtime` | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/ws/RealTimeEndpoint.java` |
| PDF generation services | Aspose-Word/PDF document generation | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/pdf/` |
| Excel services | Apache POI + Aspose Cells export | `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/excel/` |
| `graph-api` module | Microsoft Graph API integration (Azure AD user pull, scheduled sync, login) | `publicERANET-server/graph-api/src/main/java/sk/innovis/eranetpublic/server/service/` (`LoginService.java`, `UserPullService.java`, `SchedulerConfiguration.java`) |
...
- Contains: 15 top-level controller files plus deeply nested subdirectories for procurement, qualification, planning, evaluation, communication, and the eContracts module (`controllers/econtracts/`, 16 controllers incl. `editEcontract.js`)
...
- Location: `publicERANET-client/app/scripts/service/resources/` (171 files), `publicERANET-client/app/scripts/service/business/` (3 files)
...
- Location: `publicERANET-client/app/scripts/directives/` (53 files + `tableFilters/` subdir)
...
- Location: `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/`
...
- Location: `publicERANET-server/eranet-dao/src/main/java/sk/innovis/eranetpublic/server/dao/`
...
- Location: `publicERANET-server/eranet-domain/src/main/java/sk/innovis/eranetpublic/server/dto/`
...
4. HTTP request crosses to WAR context `/public_seps` on WildFly/JBoss (legacy GlassFish context root `/public`)
...
7. `XyzDao` uses `EntityManager` (EclipseLink, JTA data source `PublicSepsDS`) to query MySQL
...
3. Server validates via Java EE FORM login realm `PublicSepsRM`; SAML SSO via `SAMLClient` (`saml/`) — ÚPVS (`UpvsSsoService`, `@ApplicationPath("upvs")`) and AD/eID Azure AD SAML (`SsoService` `@Path("/saml")` under `SsoApplication` `@ApplicationPath("ad")`, so endpoints resolve at `/ad/saml/...`; auth-type constant `SAML_AD`)
...
- Examples: `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/CompanyService.java`, `ProcurementService.java`

**`BaseDao<T>` (server):**
- Purpose: Generic JPA CRUD; subclasses add entity-specific named queries
- Location: `publicERANET-server/eranet-dao/src/main/java/sk/innovis/eranetpublic/server/dao/BaseDao.java`
...
**Server WAR:**
- Deployment descriptor: `publicERANET-server/public-eranet/src/main/webapp/WEB-INF/web.xml` (servlet 3.0, FORM auth, session timeout 30 min)
- GlassFish deployment: `publicERANET-server/public-eranet/src/main/webapp/WEB-INF/glassfish-web.xml` (context root `/public`, alternatedocroot for client files)
- WildFly/JBoss deployment: `publicERANET-server/public-eranet/src/main/webapp/WEB-INF/jboss-web.xml` (context root `/public_seps`, security domain `PublicSepsRM`)
- JAX-RS app class: `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/ApplicationConfig.java` (`@ApplicationPath("webresources")`)
- JPA config: `publicERANET-server/eranet-domain/src/main/resources/META-INF/persistence.xml` (persistence unit `PublicTestPU`, JTA data source `PublicSepsDS`, EclipseLink, 303 class registrations)
- JDBC pool setup: `publicERANET-server/public-eranet/src/main/setup/glassfish-resources.xml` (MySQL, database `public`, JNDI `PublicDS`)
- CDI beans: `publicERANET-server/public-eranet/src/main/webapp/WEB-INF/beans.xml`

## Multi-Module Server Structure

The `publicERANET-server` root `pom.xml` is a **multi-module Maven reactor** (`<artifactId>eranet-parent</artifactId>`, `<packaging>pom</packaging>`) that aggregates six modules. The WAR module (`public-eranet`) depends on the library modules; all active server code lives under the module trees below, not in a top-level `src/main/java/`.

| Module | Maven role | Content |
|--------|-----------|---------|
| `public-eranet/` | WAR module (deployable) | JAX-RS/EJB services (`service/`), SSO (`sso/`), WebSocket (`ws/`), `configuration/`, `exception/`, `serialization/`, and `webapp/WEB-INF/` descriptors |
| `public-eranet-api/` | Service API | Shared service interfaces (`service/`) consumed across modules |
| `eranet-dao/` | Data-access library | `BaseDao` + entity-specific DAOs (`dao/`) |
| `eranet-domain/` | Domain library | JPA entities/DTOs (`dto/`), `serialization/`, and `persistence.xml` (`src/main/resources/META-INF/`) |
| `eranet-common/` | Shared common library | Cross-cutting shared DTO / serialization helpers |
| `graph-api/` | Microsoft Graph integration | Azure AD user pull, scheduler, and login services (`service/`) |
...
**Do this instead:** Separate JAX-RS resource classes (thin HTTP layer) from EJB service classes (business logic). Reference: `publicERANET-server/public-eranet/src/main/java/sk/innovis/eranetpublic/server/service/BaseService.java`
...
**Do this instead:** Create separate response DTOs for REST layers. Reference existing DTO face interfaces as a starting point: `publicERANET-server/eranet-domain/src/main/java/sk/innovis/eranetpublic/server/dto/face/`
...
**Authentication:** Java EE FORM-based login; roles: `administrator`, `user`, `supplier`, `statutory`, `confirmancePerson`, `responsiblePerson`, `systemAdministrator`; SAML 2.0 (`java-saml 2.2.0`, `opensaml 2.6.4`) — ÚPVS and Azure AD (AD/eID) channels
