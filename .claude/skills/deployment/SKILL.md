...
- **Build:** Maven (`mvn clean package` produces `publicERANET-server/target/eranet-server-vsetest-1.0-SNAPSHOT.war`).
...
```bash
cp publicERANET-server/target/eranet-server-vsetest-1.0-SNAPSHOT.war \
   <WILDFLY_HOME>/standalone/deployments/
```
WildFly detects the new WAR and redeploys automatically. Check the log for
`Deployed "eranet-server-vsetest-1.0-SNAPSHOT.war"`.
...
| Symptom | Fix |
|---------|-----|
| WAR won't start, JNDI lookup fails | Check WildFly data source `EranetVseTestDS` is configured and MySQL is up |
