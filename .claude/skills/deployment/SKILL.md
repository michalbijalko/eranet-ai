---
name: deploy
description: PSK release process - Maven WAR build, WildFly deployment, and Liquibase migrations. Invoke manually when deploying a change. Do NOT run automatically.
...
- **Build:** Maven (`mvn clean package` produces `publicERANET-server/target/eranet-server-1.0-SNAPSHOT.war`).
...
```bash
cp publicERANET-server/target/eranet-server-1.0-SNAPSHOT.war \
   <WILDFLY_HOME>/standalone/deployments/
```
WildFly detects the new WAR and redeploys automatically. Check the log for
`Deployed "eranet-server-1.0-SNAPSHOT.war"`.
...
| Symptom | Fix |
|---------|-----|
| WAR won't start, JNDI lookup fails | Check WildFly data source `EranetDS` is configured and MySQL is up |
